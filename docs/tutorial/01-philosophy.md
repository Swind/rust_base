...
> 本章 Chromium 素材：[`threading_and_tasks.md`](https://chromium.googlesource.com/chromium/src/+/main/docs/threading_and_tasks.md)
> 的 Overview、Quick start guide、Core Concepts、Threading Lexicon 各節。
> 本章 Rust 素材：Rust Book [ch4 所有權](https://rust-lang.tw/book-tw/ch04-00-understanding-ownership.html)、
> [ch16-01 執行緒](https://rust-lang.tw/book-tw/ch16-01-threads.html)、
> [ch16-02 訊息傳遞](https://rust-lang.tw/book-tw/ch16-02-message-passing.html)、
> [ch16-03 共享狀態](https://rust-lang.tw/book-tw/ch16-03-shared-state.html)、
> [ch16-04 Sync 與 Send](https://rust-lang.tw/book-tw/ch16-04-extensible-concurrency-sync-and-send.html)。
...
（[Rust Book ch4-01](https://rust-lang.tw/book-tw/ch04-01-what-is-ownership.html)）：
...
`std::thread::spawn` 實作的（[Rust Book ch16-01](https://rust-lang.tw/book-tw/ch16-01-threads.html)），
...
- **執行期檢查**：`SEQUENCE_CHECKER` / `THREAD_CHECKER`
  （[`base/sequence_checker.h`](https://source.chromium.org/chromium/chromium/src/+/main:base/sequence_checker.h)）
  ——只在 debug build 生效，寫錯了要等程式跑起來、而且剛好踩到，才會炸。

### Rust 怎麼防：兩個編譯期 marker trait

Rust 把這件事交給兩個 trait
（[Rust Book ch16-04](https://rust-lang.tw/book-tw/ch16-04-extensible-concurrency-sync-and-send.html)）：
...
`Rc<T>`（引用計數指標，[ch15-04](https://rust-lang.tw/book-tw/ch15-04-rc.html)）的計數
...
`RefCell<T>`（[ch15-05](https://rust-lang.tw/book-tw/ch15-05-interior-mutability.html)）
...
[ch16-02 用 channel 傳訊息](https://rust-lang.tw/book-tw/ch16-02-message-passing.html)、
[ch16-03 共享狀態（Mutex）](https://rust-lang.tw/book-tw/ch16-03-shared-state.html)，
並引用 Go 的口號：
...
- Chromium 對「為什麼偏好 sequence 而非 thread」的完整論述：
  [`threading_and_tasks.md`](https://chromium.googlesource.com/chromium/src/+/main/docs/threading_and_tasks.md) 的 *Prefer Sequences to Physical Threads* 一節。
- Rust 如何看待「可組合的並行抽象」：
  [ch16-04](https://rust-lang.tw/book-tw/ch16-04-extensible-concurrency-sync-and-send.html)
