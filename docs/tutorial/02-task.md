...
> 本章 Chromium 素材：`reference/threading_and_tasks.md` 的 Tasks 一節、
...
> [`reference/base/task/thread_pool/task.h`](../../reference/base/task/thread_pool/task.h)。
> 本章 Rust 素材：Rust Book [ch13-01 closure](../../reference/book/src/ch13-01-closures.md)、
> [ch15-01 Box](../../reference/book/src/ch15-01-box.md)、
> [ch18-02 trait object](../../reference/book/src/ch18-02-trait-objects.md)、
> [ch10-03 生命週期](../../reference/book/src/ch10-03-lifetime-syntax.md)。
...
（[ch13-01](../../reference/book/src/ch13-01-closures.md)）：
...
解法是 **trait object**（[ch18-02](../../reference/book/src/ch18-02-trait-objects.md)）：
...
——容器的每個元素必須等大。`Box<T>`（[ch15-01](../../reference/book/src/ch15-01-box.md)）
...
`'static` 是生命週期（lifetime，[ch10-03](../../reference/book/src/ch10-03-lifetime-syntax.md)）
...
- **`delayed_run_time: Option<Instant>`** —— 立即任務是 `None`，延遲任務是
  ...
  〔Rust 基礎〕`Option<T>`（[ch6-01](../../reference/book/src/ch06-01-defining-an-enum.md)）
...
- Chromium callback 系統的完整文件：`reference/base/functional/callback.h`
  開頭的大段註解（C++ 那邊為了達到 Rust closure 的效果做了多少工程，值得一看）。
- closure 捕捉的底層展開（編譯器把 closure 變成匿名 struct）：
  [ch13-01](../../reference/book/src/ch13-01-closures.md) 後半。
