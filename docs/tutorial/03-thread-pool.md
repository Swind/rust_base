...
> 本章 Chromium 素材：`reference/threading_and_tasks.md` 的 Posting a Parallel
...
> 本章 Rust 素材：Rust Book [ch6 enum 與 match](../../reference/book/src/ch06-00-enums.md)、
> [ch5 struct](../../reference/book/src/ch05-00-structs.md)、
> [ch10-02 trait](../../reference/book/src/ch10-02-traits.md)、
> [ch15-04 Rc/Arc](../../reference/book/src/ch15-04-rc.md)。
...
（[ch6-01](../../reference/book/src/ch06-01-defining-an-enum.md)）是完整的
...
**窮盡性檢查**是 `match`（[ch6-02](../../reference/book/src/ch06-02-match.md)）
...
（[ch5-01](../../reference/book/src/ch05-01-defining-structs.md)）：
...
`base::TaskRunner`（`reference/base/task/task_runner.h`）就是這個介面；文件說
它的價值在於：
...
（[ch10-02](../../reference/book/src/ch10-02-traits.md)）做同一件事但解耦更徹底：
...
（`reference/base/task/thread_pool/task_source.h`）。
...
`Arc<T>`（[ch15-04 講 `Rc`](../../reference/book/src/ch15-04-rc.md)，`Arc` 是
其原子版）是引用計數的共享指標，等價 Chromium 的 `scoped_refptr<T>` /
`base::RefCountedThreadSafe`（`reference/base/memory/scoped_refptr.h`）：
...
2. 把一個 `TaskPriority` 的 `match` 故意少寫一個 variant，讀編譯錯誤；再把
   ...
   （[ch6-03](../../reference/book/src/ch06-03-if-let.md)）。

## 延伸閱讀

- Chromium `task_traits.h`（`reference/base/task/task_traits.h`）：每個 trait
